...
public class ExempleJavaFXAvecCanvas extends Application {
	...
	public void start(Stage primaryStage) {
		...
		/*
		 * Exemple illustratif : - quelques composants graphiques, une zone de dessin et
		 * du texte - quelques conteneurs pour contenir ces composants, des boites - une
		 * fenêtre dont le contenu est déterminé par un conteneur (c'est forcément un
		 * conteneur) On peut remarquer que l'agencement des composants graphiques est
		 * déterminé par les conteneurs, qui sont toujours associé à un plan
		 * d'organisation sous jacent (Layout)
		 */
		...
		InputStream in = ClassLoader.getSystemClassLoader().getResourceAsStream("Images/Player/player_03.png");
		img = new Image(in);
		...
		// ChangeListener<Number> ecouteurRedimensionnement = new
		...
		ChangeListener<Number> ecouteurRedimensionnement = (ObservableValue<? extends Number> observable,
